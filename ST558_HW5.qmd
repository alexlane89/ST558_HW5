...
glmFit1 <- glm(Heart_Disease ~ Age, data = heartTrain, method = "glm",
  control = trctrl,
  preProcess = c("center", "scale"))
```
> Model 2:
...
#interaction effects.
glmFit2 <- glm(formula = Heart_Disease ~ Age*Cholesterol,
               data = heart_3,
               family = "binomial")
summary(glmFit2)
```
...
glmFit3 <- glm(formula = Heart_Disease ~ Age + Cholesterol + Sex,
               data = heart_3,
               family = "binomial")
summary(glmFit3)
...
```{r}
glm1_fit <- train(Heart_Disease ~., data = heartTrain, method = "knn",
 trControl=trctrl,
 preProcess = c("center", "scale"),
 tuneLength = 10)
```
